...
class WebGuiSession:
    ...
    def _run_script(self, script_name: str, check_output: bool = True) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f'Script {script_name} failed with code {e.returncode}: {error_msg}'
            )
    ...
    def _wait_for_service(self, service_name: str, timeout: int = 30) -> bool:
        ...
        logger.warning(
            f'Service {service_name} failed to start within {timeout} seconds'
        )
        return False
            ...
            try:
                ...
                if not os.path.exists(self.gui_home):
                    raise FileNotFoundError(
                        f'GUI_HOME directory not found: {self.gui_home}'
                    )
                ...
                if not self._wait_for_port(self.novnc_port, timeout=10):
                    raise RuntimeError(
                        f'noVNC web service failed to start on port {self.novnc_port}'
                    )

                self.status = GuiSessionStatus.RUNNING
                self._initialized = True

                logger.info('GUI extension services started successfully')
    ...
    def _wait_for_port(self, port: int, timeout: int = 30) -> bool:
        ...
        logger.warning(
            f'Port {port} failed to start listening within {timeout} seconds'
        )
        return False
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        status_info = {
            ...
            'ports': {'vnc': self.vnc_port, 'novnc': self.novnc_port},
            'services': {},
        }
        ...
        # # 检查端口状态
        # status_info['ports']['vnc_listening'] = self._check_port_listening(
        #     self.vnc_port
        # )
        # status_info['ports']['novnc_listening'] = self._check_port_listening(
        #     self.novnc_port
        # )
...
# 便利函数
def start_gui_session(gui_home: str = '', **kwargs) -> WebGuiSession:
    """启动GUI会话的便利函数
