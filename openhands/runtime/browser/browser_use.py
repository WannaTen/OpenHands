...
class BrowserUseEnv:
    ...
    def browser_process(self) -> None:
        ...
        try:
            ...
            browser_context = loop.run_until_complete(
                browser_instance.new_context(config=context_config)
            )
            ...
            loop.run_until_complete(
                self._navigate_to_url(browser_context, 'about:blank')
            )
                    ...
                    try:
                        if action.startswith('goto:'):
                            ...
                            result = loop.run_until_complete(
                                self._navigate_to_url(browser_context, url)
                            )
                        elif action.startswith('click:'):
                            element_index = int(action[6:])
                            result = loop.run_until_complete(
                                self._click_element(browser_context, element_index)
                            )
                            if 'error' in result:
                            ...
                            if len(parts) == 2:
                                ...
                                result = loop.run_until_complete(
                                    self._input_text(
                                        browser_context, element_index, text
                                    )
                                )
                                if 'error' in result:
                                    action_execution_error_message = result['error']
                            ...
                            if direction == 'down':
                                result = loop.run_until_complete(
                                    self._scroll_down(browser_context)
                                )
                            else:
                                result = {'error': '滚动方向无效'}
                        else:
                        ...
                        obs = loop.run_until_complete(
                            self._get_observation(browser_context, result)
                        )
                        if action_execution_error_message:
                            obs['error_message_for_step'] = (
        ...
        while True:
            if self.agent_side.poll(timeout=0.01):
                ...
                if response_id == unique_request_id:
                    ...
                    final_obs.setdefault(
                        'open_pages_urls',
                        [final_obs['url']] if final_obs.get('url') else [],
                    )
                    ...
                    # 处理错误信息和动作相关字段
                    final_obs['last_action'] = (
                        action_str  # utils.py 会用这个作为 last_browser_action
                    )

                    error_message_from_proc = final_obs.pop(
                        'error_message_for_step', None
                    )
